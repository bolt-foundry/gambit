...
export async function exportVideo(
  ...
  try {
    ...
    logger.warn("[gambit-demo] failed to read frames directory:", error);
  }
  if (!hasFrames) return;
  try {
    ...
    if (code !== 0) {
      ...
      logger.warn("[gambit-demo] ffmpeg failed:", message || code);
    }
  } catch (error) {
    if (error instanceof Deno.errors.NotFound) {
      logger.warn("[gambit-demo] ffmpeg not found; keeping frame PNGs");
    } else {
      logger.warn("[gambit-demo] ffmpeg failed:", error);
    }
  }
}

export async function transcodeMediaRecorderVideo(
  inputPath: string,
  outputPath: string,
): Promise<boolean> {
  try {
    ...
    if (code !== 0) {
      ...
      logger.warn(
        "[gambit-demo] MediaRecorder transcode failed:",
        message || code,
      );
      return false;
    }
    ...
    await Deno.remove(outputPath).catch(() => {});
    if (error instanceof Deno.errors.NotFound) {
      logger.warn("[gambit-demo] ffmpeg not found; keeping MediaRecorder webm");
    } else {
      logger.warn("[gambit-demo] MediaRecorder transcode failed:", error);
    }
    return false;
  }
}
...
export async function trimMediaForAudioDelay(opts: {
  ...
  try {
    ...
    if (code !== 0) {
      const message = new TextDecoder().decode(stderr).trim();
      logger.warn("[gambit-demo] video trim failed:", message || code);
      return;
    }
  } catch (error) {
    logger.warn("[gambit-demo] video trim failed:", error);
    return;
  }
  ...
  if (audioPath && await fileExists(audioPath)) {
    ...
    try {
      ...
      if (code !== 0) {
        const message = new TextDecoder().decode(stderr).trim();
        logger.warn("[gambit-demo] audio trim failed:", message || code);
        trimmedAudioPath = null;
      }
    } catch (error) {
      logger.warn("[gambit-demo] audio trim failed:", error);
      trimmedAudioPath = null;
    }
  }
  ...
  if (trimmedAudioPath) {
    ...
    try {
      ...
      if (code !== 0) {
        const message = new TextDecoder().decode(stderr).trim();
        logger.warn("[gambit-demo] trimmed mux failed:", message || code);
      }
    } catch (error) {
      logger.warn("[gambit-demo] trimmed mux failed:", error);
    }
  }
}
