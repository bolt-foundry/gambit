...
export default defineDeck({
  ...
  contextSchema: z.object({
    path: z.string().describe("Relative directory path under the bot root."),
  }),
  ...
  async run(ctx) {
    ...
    try {
      ...
      if (stat.isDirectory) {
        return {
          status: 200,
          payload: { path: resolved.relativePath, created: false },
        };
      }
    ...
    try {
      ...
      return {
        status: 201,
        payload: { path: resolved.relativePath, created: true },
      };
    } catch (err) {
      return {
        status: 500,
