...
function normalizeSchema(schema?: ZodTypeAny): NormalizedSchema | undefined {
  ...
  switch (typeName) {
    ...
    case "ZodNativeEnum": {
      const values = (core as { _def: { values: Record<string, unknown> } })._def
        .values;
    ...
    case "ZodObject": {
      ...
      const shape = (core as { _def: { shape: () => Record<string, ZodTypeAny> } })
        ._def.shape();
...
function unwrapSchema(schema: ZodTypeAny): {
  ...
  while (current && typeof current === "object") {
    const def = (current as { _def?: { typeName?: string; [k: string]: unknown } })
      ._def;
