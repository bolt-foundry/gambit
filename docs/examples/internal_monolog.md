...
- `internal_monolog_parent.deck.md`: root deck that calls the child tool and
  relays its answer.
- `monolog_child.deck.md`: LLM child with the `gambit://respond` marker that
...
```bash
deno run -A src/cli.ts serve examples/advanced/cli_internal_monolog/internal_monolog_parent.deck.md --model openai/gpt-4o-mini --trace out.jsonl --verbose
```
