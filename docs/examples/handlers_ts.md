...
- Trigger busy/idle:
  `deno run -A src/cli.ts run init/examples/advanced/cli_handlers_ts/handlers_ts.deck.md --context '{"text":"please wait","delayMs":2500}' --stream`
- Trigger error handling: `--context '{"text":"fail this","fail":true}'`

Try this input

- `--context '{"text":"run slow","delayMs":2500}'` → calls `slow_action_ts`,
  busy handler streams status, idle may fire, final echo after delay
- `--context '{"text":"fail this","fail":true}'` → calls `flaky_action_ts`,
